...
$$Y = \exp\{ - \left( W_{1} - 1 \right)^{2} - \left( W_{2} - 1 \right)^{2})\} - 2\exp\{ - \left( W_{1} + 1 \right)^{2} - \left( W_{2} + 1 \right)^{2}\} A + \epsilon$$
with independent measurement error $\epsilon \sim \mathcal{N}(0,1)$,
treatment variable
$A \sim Bernoulli\left( \text{expit}\{ - 1 + W_{1}\} \right)$ and
independent covariates $W_{1},W_{2} \sim \mathcal{N}(0,1/2)$.
...
interaction term between treatment and the two covariates $W_{1}$ and
$W_{2}$
...
Predictions, $E\left( Y \mid W_{1},W_{2} \right)$, can be performed with
the `predict` method
