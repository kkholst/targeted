...
#ifndef SRC_TARGETED_H_
#define SRC_TARGETED_H_
...
namespace targeted {
  ...
  */
  ...
  public:
    ...
    Targeted(const arma::Col<T> &y, const arma::Mat<T> &a,
  ...
  template <class T>
  class TargetedBinary : public Targeted<T> {
  protected:
    arma::Mat<T> pr;
    virtual arma::Col<T> H() = 0;
    virtual arma::Mat<T> dp() = 0;
  public:
    using Targeted<T>::Targeted;
  ...
  template <class T>
  class RD : public TargetedBinary<T> {
  private:
  ...
  template <class T>
  class RR : public TargetedBinary<T> {
  private:
  ...
  class ACE : public Targeted<Complex> {
  protected:
    bool binary;
