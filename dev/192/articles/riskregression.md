...
$`\mu_{RR}`$ has been set to `lp.target =` 1.
...
(i.e., the matrix $`W`$ at the start of this text), and the nuisance
model design matrix `x2` (odds product).
...
introduces a model for the exposure $`E(A=1\mid V)`$ (propensity model).
The double-robustness stems from the fact that the this estimator
remains consistent in the union model where either the odds-product
model or the propensity model is correctly specified. With both models
correctly specified the estimator is efficient.
...
\log RR(V) = A \alpha^TV
```
for a subset $`V`$ of the covariates can be estimated using the `target`
argument:
...
```
                ...
                Estimate Std.Err   2.5% 97.5% P-value
    (Intercept)    1.039 0.02114 0.9978 1.081       0
...
```
where $`Z_i = (Y_i, A_i, V_i), i=1,\ldots,n`$ are the i.i.d.
observations and $`\phi_{\mathrm{eff}}`$ is the *influence function*.
