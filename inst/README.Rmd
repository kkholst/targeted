---
title: "`targeted`: Targeted Learning in R"
output:
  html_document:
    toc: true
    toc_depth: 2
...
simdata <- function(n, ...) {
   ...
   y <-  3 * exp(- (w1-1)**2 - (w2+1)**2) + # continuous
     - 3 * exp(- w1**2 - w2**2) * a +
     rnorm(n, sd=0.5**2)
...
As an example, we can specify a linear regression model with main effects of
$W_1$ and $W_2$
```{r learner1}
lr <- learner_glm(y ~ w1 + w2, family = gaussian)
lr
```
...
Similarly, a flexible gradient boosting model can be specified with
```{r lrxgb}
lr2 <- learner_xgboost(
  y ~ w1 + w2,
...
Lists of models can also be constructed for different hyperparameters with the
...
```{r   }
m <- lava::lvm() |>
    lava::regression(a ~ x+z) |>
    lava::regression(lp.target ~ 1) |>
    lava::regression(lp.nuisance ~ x + z) |>
    lava::distribution('a', binomial.lvm("logit")) |>
    lava::binomial.rr('y', 'a', 'lp.target', 'lp.nuisance')

par <- c('a'=-2, 'lp.target'=1, 'lp.nuisance'=-1, 'lp.nuisance~x'=2)
d <- lava::sim(m, n=2e4, seed=1, p=par) |>
