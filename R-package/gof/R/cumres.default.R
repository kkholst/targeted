subset_t <- function(t, n.qt=500L, n.eq=500L) {
    ...
    idx1 <- setdiff(c((.clusterindex(idx)[,1]-1)[-1],length(idx)),0)
...
`cumres.default` <-
    function(model, variable, r=stats::residuals(model), dr,
      ...
      ...) {
        ...
        if (!is.null(subset.max) && n>subset.max && !is.null(subset) && n>5e3)  {
