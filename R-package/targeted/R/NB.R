...
##  ##m1 <- condlogit(Species ~ Sepal.Width + Petal.Length, data=iris)##
##  ##pr1 <- predict(m1, newdata=iris, wide=TRUE)##
##  ##ScoringMany(iris$Species, mlogit=pr1, NB=pr2)
NB <- function(formula, data, weights=NULL,
    ...
    if (!is.data.table(data)) data <- data.table::data.table(data)
    ...
    estcond <- function(x,weights,...) {
        if (is.data.table(x)) x <- as.matrix(x[,1])
