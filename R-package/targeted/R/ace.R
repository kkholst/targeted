...
ace <- function(formula, exposure=NULL,
         data, weights, binary=TRUE,
         all, missing=FALSE, labels=NULL, ...) {
    if (is.list(formula)) {        
        yf <- getoutcome(formula[[1]])
        if (is.null(exposure)) exposure <- gsub("`","",attr(yf,"x"))
        xf <- formula
        xf[[1]] <- update(xf[[1]], .~.-1)
    } else {
    ...
    if (!is.null(exposure) && inherits(exposure,"formula")) {
        xf <- c(xf, list(exposure))
    }
