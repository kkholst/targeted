...
i.e., When marginalizing we leave the class of logistic
regression. This non-collapsibility makes it hard to interpret
odds-ratios and to compare results from *different studies*
...
**nuisance model** for the *odds-product*
...
```{r, fig.width=5, fig.height=5, fig.caption=' '}
  ...
  for (rr in exp(seq(-3,3,by=.25))) lines(p0,p1(p0,rr), col=2)
```
...
[(CRAN)](https://cran.r-project.org/web/packages/lava/)
...
\(\mu_{RR}\) **
has been set to `lp.target =` `r p['lp.target']`.
...
estimator asymptomatically attains the semi-parametric efficiency bounds.
