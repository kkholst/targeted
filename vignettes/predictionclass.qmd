...
Many of the following built-in learners use the [logger](https://daroczig.github.io/logger/index.html) package to inform users about non-critical warnings and errors. The behavior of the logger can be controlled by updating the [log threshold](https://daroczig.github.io/logger/reference/log_levels.html) and be disabled with `log_threshold(OFF)`. Note that the `{targeted}` packages imports `{logger}`.
...
predictions, for example, to prevent predictions with extreme values or to
handle rows for which the prediction function fails.
